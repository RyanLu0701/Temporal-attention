...
def count(data):
    ...
    out_dim1 = check(data[0],4)
    out_dim2 = check(data[1],32)
    out_dim3 = check(data[2],32)
...
class trans(nn.Module):
    def __init__(self,nhead,num_layers,d_model):
        super().__init__()
...
class my_model(nn.Module):
    def __init__(self,
        ...
        self.date2vec = Date2Vec(input_dim=2,k=xshape[1]*xshape[2])
        self.swin = SwinTransformer3D(patch_size,embed_dim,depths,num_heads,window_size)
        self.trans = trans(nhead,num_layers,dim)

    def forward(self,x,date):
        date_embed = self.date2vec(date)
        print(date_embed.shape)
        date_embed =date_embed.reshape(x.shape[0],x.shape[2],x.shape[3],x.shape[4])

        for i in range(x.shape[0]):
            for j in range(x.shape[1]):
                x[i,j] = x[i,j] + date_embed[i]
